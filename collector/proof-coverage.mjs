...
export function calculateProofFunnel(state, now = new Date()) {
  ...
  return {
    cohort:{capturedAt:state.proofCoverageCohort?.capturedAt,expiresAt:state.proofCoverageCohort?.expiresAt,size:cohort.size,retained:pools.length,missing:cohort.size-pools.length},
