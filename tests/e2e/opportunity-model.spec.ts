...
test.describe("pool market to token opportunity contracts", () => {
  ...
  test("rejects missing, invalid, future, and 136-day timestamps from New on Base", async () => {
    const template = await fixtureTemplate();
    const now = Date.now();
    const valid = verifiedPool(fixturePool(template, { poolAddress: address(401), tokenAddress: address(501), tokenSymbol: "NEW", quoteAddress: USDC, quoteSymbol: "USDC", createdAt: now - 60_000 }));
