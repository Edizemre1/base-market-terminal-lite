...
export function BaseTerminal({
  ...
  return <main id="terminal-main" tabIndex={-1} className="min-h-[calc(100vh-56px)] w-full scroll-mt-16 overflow-x-hidden bg-surface-canvas px-3 py-3 outline-none sm:px-4 lg:px-6" data-testid="pulse-terminal" data-terminal-view={view}><h1 className="sr-only">{viewTitle}</h1>
    <MarketSignalProvider snapshot={snapshotData}><TradeabilityProvider><div className="mx-auto max-w-[2200px] space-y-3">
      ...
      {view === "workspace" ? <section className="space-y-3" data-testid="pair-workspace"><div className="flex items-center justify-between gap-2 px-1"><div><p className="text-meta font-bold uppercase tracking-eyebrow text-content-secondary">{t("workspace.selected")}</p><h1 className="text-base font-semibold">{selectedPairWithLiveChart.focusTokenSymbol ?? selectedPairWithLiveChart.baseToken}</h1></div><button type="button" onClick={() => navigateView("markets")} className="min-h-10 rounded-control bg-surface-interactive px-3 text-meta text-content-secondary">{t("common.backToMarkets")}</button></div><div className="grid min-w-0 items-start gap-3 cmi-inspector-grid"><div className="min-w-0 space-y-3"><SelectedPairPanel pair={selectedPairWithLiveChart} marketDataMode={snapshotData.mode} chartRefreshStatus={chartRefreshStatus[getChartCacheKey(selectedPairWithLiveChart)] ?? "idle"} onRefreshChart={refreshPairChart} /><MarketActivityPanel pair={selectedPairWithLiveChart} signals={pulseSignals} snapshot={snapshotData} /><PairDetailTabs pair={selectedPairWithLiveChart} activeTab={activeTab} onTabChange={setActiveTab} providerStale={providerHealth.stale} /></div>{!tradeDrawerOpen ? <div role="region" className="sticky top-shell-header hidden max-h-sheet-max min-w-0 overflow-y-auto lg:block" data-testid="workspace-trade-panel" aria-label={t("trade.dock")}><TradeDock pair={selectedPairWithLiveChart} marketDataMode={snapshotData.mode} amount={amount} onAmountChange={setAmount} side={tradeSide} onSideChange={setTradeSide} onInteractionChange={(locked) => setInteractionLock("trade", locked)} pollCapabilities={false} /></div> : null}</div><div className="sticky bottom-shell-header z-layer-sticky rounded-card border border-border-subtle bg-surface-panel/95 p-2 backdrop-blur-xl lg:hidden" data-testid="workspace-mobile-trade-cta"><button type="button" onClick={() => openTrade(selectedPairWithLiveChart, "buy")} className="min-h-control-touch w-full rounded-control bg-brand-action px-6 text-label font-bold text-content-on-accent">{t("trade.checkQuote")}</button></div></section> : null}
      ...
      {tradeDrawerOpen ? <TradeDrawer onClose={overlay.close} suspended={overlay.active.type !== "trade_drawer"}><TradeDock pair={tradePair ?? selectedPairWithLiveChart} marketDataMode={snapshotData.mode} amount={amount} onAmountChange={setAmount} side={tradeSide} onSideChange={setTradeSide} onInteractionChange={(locked) => setInteractionLock("trade", locked)} /></TradeDrawer> : null}
...
function TradeDrawer({ onClose, children, suspended = false }: { onClose: () => void; children: ReactNode; suspended?: boolean }) {
  const { t } = useI18n();
  return <div className="fixed inset-0 z-layer-drawer flex items-end justify-end bg-surface-scrim/75 lg:bg-surface-scrim/35" aria-hidden={suspended || undefined} onMouseDown={(event) => { if (!suspended && event.target === event.currentTarget) onClose(); }}><div role="dialog" aria-modal="true" aria-label={t("trade.dock")} data-overlay-root="trade_drawer" className={cx("flex h-[calc(100dvh-56px)] max-h-[calc(100dvh-56px)] w-full flex-col overflow-hidden rounded-t-overlay bg-surface-panel px-2 pt-2 text-content-primary shadow-overlay lg:h-full lg:max-h-none lg:w-inspector lg:max-w-inspector lg:rounded-l-overlay lg:rounded-tr-seam lg:border-l lg:border-border-subtle lg:p-3", suspended && "pointer-events-none")}><button type="button" onClick={onClose} className="mb-2 ml-auto grid h-11 w-11 shrink-0 place-items-center rounded-pill bg-surface-interactive text-content-secondary" aria-label={t("trade.closeDock")} data-overlay-autofocus><X size={16} aria-hidden="true" /></button><div className="min-h-0 w-full flex-1 overflow-y-auto cmi-safe-footer" data-testid="trade-drawer-content">{children}</div></div></div>;
}
