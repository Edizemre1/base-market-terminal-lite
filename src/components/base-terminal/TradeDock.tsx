...
export function TradeDock({ pair, marketDataMode, amount, onAmountChange, side, onSideChange, onInteractionChange }: {
  ...
  useEffect(() => {
    setWalletSpendToken({ ...SPEND_TOKENS[spendToken], decimals: SPEND_TOKEN_DECIMALS[spendToken] });
  ...
  const balance = balanceRaw !== undefined && quote?.fromToken ? formatRawTokenAmount(balanceRaw, quote.fromToken.decimals) : undefined;
