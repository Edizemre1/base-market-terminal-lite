...
export function TradeDock({ pair, marketDataMode, amount, onAmountChange, side, onSideChange, onInteractionChange }: {
  ...
  const connected = wallet.status === "connected" && Boolean(wallet.address) && wallet.chainId === BASE_CHAIN_ID;
  ...
  async function openReview() {
    ...
    try {
      ...
      setReviewOpen(true); setTransactionStatus("idle");
  ...
  return <aside className="pulse-surface min-w-0 overflow-hidden rounded-xl" data-testid="trade-dock" onFocusCapture={() => onInteractionChange(true)} onBlurCapture={(event) => { if (!event.currentTarget.contains(event.relatedTarget as Node | null)) onInteractionChange(false); }}>
