...
export function LiveMarketWall({ snapshot, selectedPair, onSelect, onTrade, onInteractionChange }: {
  ...
  return <section className="min-w-0" data-testid="live-market-wall" data-live-wall-timeframe={timeframe} data-visible-opportunities={wall.visibleOpportunityCount} data-cross-lane-duplicates={wall.duplicateCount} onMouseEnter={() => { hoverRef.current = true; syncLock(); }} onMouseLeave={() => { hoverRef.current = false; syncLock(); }} onFocusCapture={() => { focusRef.current = true; syncLock(); }} onBlurCapture={(event) => { if (!event.currentTarget.contains(event.relatedTarget as Node | null)) { focusRef.current = false; syncLock(); } }}>
    <header className="mb-2 flex flex-wrap items-end justify-between gap-2 px-1">
      <div><p className="text-meta font-bold uppercase tracking-eyebrow text-content-secondary">{t("terminalV3.wall")}</p><h2 className="mt-1 text-title-sm font-semibold text-content-primary">{t("terminalV3.wallSubtitle")}</h2><p className="mt-1 flex flex-wrap gap-2 font-mono text-meta text-content-secondary"><span data-testid="wall-visible-unique">{t("terminalV3.visibleUnique", { count: wall.visibleOpportunityCount })}</span><span data-testid="wall-duplicate-count">{t("terminalV3.duplicateCount", { count: wall.duplicateCount })}</span></p></div>
...
function LiveWallLaneCard({ lane, selectedPair, snapshot, highlighted, liquidityDirection, onLiquidityDirection, onSelect, onTrade, formatCompactCurrency, formatPercent, locale }: {
  ...
  return <article className={cx("live-wall-lane pulse-surface overflow-hidden rounded-card", laneAccent(lane.id))} data-testid={`live-wall-lane-${lane.id}`} data-lane-count={lane.entries.length} data-lane-eligible={lane.eligibleCount} data-lane-fallback={lane.fallback || undefined} data-lane-freshness={lane.freshness}>
    <header className="flex min-h-12 items-start justify-between gap-2 px-3 pb-1 pt-2"><div className="min-w-0"><p className="truncate text-label font-semibold text-content-primary">{title}</p><p className="mt-1 flex items-center gap-2 font-mono text-meta text-content-secondary"><span className={cx("h-2 w-2 rounded-pill", lane.freshness === "fresh" ? "bg-freshness-live" : lane.freshness === "delayed" ? "bg-freshness-delayed" : "bg-content-secondary")} />{lane.baselinePending ? t("terminalV3.baselinePending") : `${lane.eligibleCount} · ${lane.timeframe === "age" ? "7d" : lane.timeframe === "snapshot" ? `${snapshot.comparison.previousGeneratedAt ? "Δ" : "—"}` : displayWindow(lane.timeframe)}`}</p></div><Link href={`/terminal?view=markets&data=${snapshot.mode}`} className="inline-flex min-h-8 shrink-0 items-center gap-1 px-2 text-meta font-semibold text-content-secondary hover:text-content-primary">{t("terminalV3.viewAll")}<ArrowRight size={12} /></Link></header>
