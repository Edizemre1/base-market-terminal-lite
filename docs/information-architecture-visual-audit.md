...
| Trade controls occupied desktop space before explicit intent | Inspect is always available; ranking-eligible rows say “Check quote” until an exact fresh route exists | action layer | Permanent Trade Dock and premature Buy/Sell claims are removed from discovery surfaces | `trade_drawer` | Right drawer on desktop; independent bottom sheet on mobile; Buy/Sell remains the explicit side selector inside the trade lifecycle |
...
| Matrix showed fourteen data columns by default | Market Board defaults to Market, Price, 5m, 1h, 24h, Volume, Liquidity, Age, Signals, Action | `rowPrimary` | Provider, freshness, combined data state, trade state, pools, FDV, and trades are persistent opt-in columns | `columns` | Numeric cells do not wrap; cards replace the table below the desktop breakpoint and preserve price/change/activity/status/action order |
