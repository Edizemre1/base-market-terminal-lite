...
3. Pool code and available token/pool/factory getters are checked at the event block. If a non-archive RPC cannot serve historical bytecode, code alone is retried at `latest` while identity getters remain pinned to the event block; the retained binding kind exposes that fallback. Metadata failures degrade to partial or unavailable.
...
New confirmed pools and prioritized unpriced opportunities enter a bounded queue. Each normal enrichment batch reserves three quarters of its capacity for never-enriched or pending pools and one quarter for matched-pool refreshes, preventing high-priority refresh work from starving exact-address coverage while preserving freshness. The collector requests the exact DexScreener Base pair plus the exact GeckoTerminal Base pool and pool-info endpoints. It never resolves by symbol. Requests use provider-specific timeouts and rate gates, bounded concurrency, retry/backoff, circuit breakers, in-flight coalescing, a short positive cache, and a longer negative-result TTL. A request queued behind a provider rate gate rechecks the circuit before network I/O, so a concurrent 429 cannot multiply retries or push the batch into a growing slot backlog. A recently confirmed provider miss remains pending for bounded retry; an older exact 404 is retained as not found. Provider observations merge into one canonical pool with field-level provenance, so liquidity, volume, and transactions are not double-counted.

## Store and limits

The dependency-free schema-v1 store contains factory cursors, canonical events, pools, metadata snapshots, token opportunities, restart-safe provider/on-chain/metadata queues, provider field provenance, price-anchor and market-snapshot slots, reconciliation history, replay evidence, and a bounded relay ring. Successful decimal metadata is immutable in normal collection; failed reads retain a reason and bounded retry time. The 2,000-pool cap protects at most 512 exact provider-matched pools before filling the remaining capacity with the newest detections, preventing source-positive coverage from being silently evicted without making the store unbounded. It uses a single-owner lock, a bounded serialized transaction queue, fsynced hash-chained delta prepare/commit records, sparse atomic checkpoints, SHA-256 integrity, deterministic fail-closed replay, retention caps, and clean signal shutdown. Status-only and no-op cycles do not write the checkpoint or WAL. Web readers keep a verified replay cursor and consume only appended WAL bytes. Exact limits, recovery rules and canary gates are defined in `docs/collector-resource-recovery.md`.
...
Default limits include 250 blocks per log query, 2,000 bootstrap blocks, four chunks per pass, one metadata job and one RPC-backed on-chain state job per public-RPC-safe cycle, 128 local pending/unsupported classifications, a 45,000 ms state-cycle deadline, a 3,000 ms public-RPC evidence-batch start interval, 2,000 pools, 5,000 canonical events, 256 relay events, 512 history records, 128 reconciliation records, 256 metadata jobs, and 64 SSE clients. Metadata-pending and unsupported pools are reason-coded without spending an RPC head read; a ready pool still consumes the single RPC-backed slot. The first evidence batch proceeds immediately; later batches wait only for the unelapsed part of that interval. Verified manager bytecode evidence for manager-based PoolId protocols is reused for the bounded life of the collector process; a restart must prove it again, while address-based pools remain individually bound. Historical bytecode is preferred; a retryable archive-gap response triggers one paced `latest` bytecode fallback, recorded as `manager_pool_id_latest_code` or `factory_event_and_latest_code`, while token/factory getters stay at the exact event block. A mixed batch with successful bytecode proof and reason-coded getter failures neither retries the successful calls nor opens the transport circuit; only an entirely retryable-failed batch is retried and counts toward that circuit. The discovery poll interval is also a start-to-start cadence: time already spent in a bounded scan is subtracted from the next idle delay, with a 3,000 ms public-RPC safety floor, so verification work does not add a second full poll interval to cursor lag or begin the next scan as a burst. A staging-only catch-up profile may temporarily override `ONCHAIN_DISCOVERY_BATCH_PACE_MS` while keeping scan concurrency and durable commits bounded; remove the override before acceptance and verify lag against an independent RPC head.
...
Before switching releases, retain the current release symlink target, web/collector unit hashes, protected-env hash, nginx hash, and the exact checkpoint/WAL pair with hashes and modes. To roll back, stop only the staging web and collector units. A cleanly stopped new collector must first have checkpointed all committed version-2 deltas; old code must never be started against WAL-only newer state. Otherwise restore the preserved pre-canary checkpoint/WAL pair. Atomically restore the prior release, then start the staging collector followed by the staging web unit. Verify the deploy SHA, store digest/schema, collector cursor, web health, Basic Auth boundary, and both unit journals. Schema v1 is unchanged by this release; no production service or data path participates.
