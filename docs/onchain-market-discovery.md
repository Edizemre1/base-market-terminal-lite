...
Default limits include 250 blocks per log query, 2,000 bootstrap blocks, four chunks per pass, a 2,000 ms public-RPC evidence-batch pace, 2,000 pools, 5,000 canonical events, 256 relay events, 512 history records, 128 reconciliation records, 256 metadata jobs, and 64 SSE clients. The discovery poll interval is a start-to-start cadence: time already spent in a bounded scan is subtracted from the next idle delay, with a 50 ms floor, so verification work does not add a second full poll interval to cursor lag. A staging-only catch-up profile may temporarily raise `ONCHAIN_DISCOVERY_BATCH_PACE_MS` while keeping scan concurrency and durable commits bounded; remove the override before acceptance and verify lag against an independent RPC head.
