...
- Wallet chrome distinguishes Connect, Reconnect, Locked, Wrong network and session-verified Connected. The Wallet drawer and Portfolio share one explicit balance surface for ETH, WETH, USDC and the selected market token. Loading, unavailable, unverified metadata and exact zero remain distinct; no complete-wallet or exact USD total is claimed without indexed holdings and verified prices.
- Buy and Sell always expose their exact spend/receive orientation. Native ETH Max reserves a bounded gas allowance, and review rechecks exact spend balance plus current Base gas before any approval or swap request. Closing review clears its simulation; no connection, balance read, quote, signature, approval or transaction starts implicitly.
