...
The staging unit safety envelope is `CPUQuota=25%`, `MemoryMax=512M`, `IOWeight=50`, and `Nice=10`. `IOWeight` is relative, not an absolute write cap. Before a canary, resolve the store's backing block device with `findmnt` and add a separately reviewed `IOWriteBandwidthMax=<exact-device> 2M` drop-in. Do not guess a device path. Canary acceptance additionally requires at most 128 MiB written in 30 minutes, a projected steady-state budget below 256 MiB/hour and 2 GiB/day, and no monotonically growing queue or WAL. Crossing any budget is an automatic stop and rollback condition.
